    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Badge variant={getStatusVariant(enrollment.status)}>
          {{ pending: "Pending", under_review: "Under Review", approved: "Approved", rejected: "Rejected", enrolled: "Enrolled" }[String(enrollment.status)] ?? String(enrollment.status).replace("_", " ")}
        </Badge>
      </div>
                ...
                <div key={`${entry.status}-${entry.changedAt}-${index}`} className="flex gap-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-semibold">
                      {{ pending: "Pending", under_review: "Under Review", approved: "Approved", rejected: "Rejected", enrolled: "Enrolled" }[String(entry.status)] ?? String(entry.status).replace("_", " ")}
                    </p>
