      ...
      <div className="rounded-[18px] border border-slate-300 bg-slate-50/80 p-3 md:p-4">
        ...
        <div className="mb-4 rounded-[14px] bg-[#e8edf5] p-3 md:p-4">
          <div className="grid gap-3 md:grid-cols-[1.4fr_0.6fr] md:items-end">
            <div className="min-w-0">
              <div className="text-[0.7rem] font-medium uppercase tracking-[0.15em] text-slate-600">To</div>
              <div className="mt-1 max-w-full truncate text-[1.3rem] font-black leading-none tracking-tight text-slate-900 md:text-[1.8rem]">{payerName}</div>
            </div>

            <div className="ml-auto w-full max-w-sm text-[0.7rem] md:text-xs">
        ...
        <div className="overflow-hidden rounded-[12px] border border-slate-200 bg-white">
          <table className="w-full table-fixed border-collapse text-left text-[0.68rem] md:text-[0.9rem]">
            <thead>
              <tr className="bg-[#1f2937] text-white">
                <th className="w-[10%] px-2 py-2 text-left text-[0.58rem] font-bold uppercase tracking-[0.14em] md:px-3">Qty</th>
                <th className="w-[46%] px-2 py-2 text-left text-[0.58rem] font-bold uppercase tracking-[0.14em] md:px-3">Description</th>
                <th className="w-[22%] px-2 py-2 text-right text-[0.58rem] font-bold uppercase tracking-[0.14em] md:px-3">Unit Price</th>
                <th className="w-[22%] px-2 py-2 text-right text-[0.58rem] font-bold uppercase tracking-[0.14em] md:px-3">Amount</th>
              </tr>
            </thead>
            <tbody>
              {items.length === 0 ? (
                ...
                items.map((it, i) => (
                  <tr key={i} className="border-b border-slate-200 bg-white last:border-b-0">
                    ...
                    <td className="max-w-0 px-2 py-3 align-top break-words text-slate-700 md:px-3">{it.description}</td>
        ...
        <div className="mt-4 flex justify-end">
          <div className="w-full max-w-[300px] rounded-[12px] border border-slate-200 bg-slate-100 p-3">
            <div className="flex items-center justify-between gap-3 text-sm font-semibold text-slate-700">
              <span>Total</span>
              <span className="text-[1.2rem] font-black tracking-tight whitespace-nowrap text-slate-900 md:text-[1.5rem]">{fmt(subtotal, currency)}</span>
            </div>
          </div>
        </div>
      </div>
